...
const LaunchTimeline = ({ changeclass }) => {
  ...
  const uniqueYears = [...new Set(launchData.map((launch) => getLaunchYear(launch.launch_date_utc)))];

  return (
    <div className={changeclass === 'Launch Timeline' ? "full-width" : "launch-timeline"}>
      {uniqueYears.map((year) => (
        <div key={year} className="timeline-year">  
          {year}
          <div className="timeline-events">
            {displayedLaunches
              .filter((launch) => getLaunchYear(launch.launch_date_utc) === year)
              .map((launch) => (
                <div key={launch.flight_number} className="timeline-event">
                  ...
                  <div className="timeline-details">
                    <div className="timeline-date">{launch.launch_date_utc}</div>
